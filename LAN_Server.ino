...
void setup() 
{
  ...
  h2 = 0;
  t2 = 0;
  byte n_pcb = EEPROM.read(0);
  int p = (int)(EEPROM.read(1))*256 + (int)(EEPROM.read(2));
  byte ip_e[4];
  byte gw_e[4];
  byte sn_e[4];
  for (int i = 0; i <= 3; i++)
  {
    ip_e[i] = EEPROM.read(3 + i);
    gw_e[i] = EEPROM.read(7 + i);
    sn_e[i] = EEPROM.read(11 + i);
  }
  if (((ip[0] != ip_e[0]) || (ip[1] != ip_e[1]) || (ip[2] != ip_e[2]) || (ip[3] != ip_e[3])) &&
    ((ip_e[0] != 0) || (ip_e[1] != 0) || (ip_e[2] != 0) || (ip_e[3] != 0)))
  {
    ip[0] = ip_e[0];
    ip[1] = ip_e[1];
    ip[2] = ip_e[2];
  ...
  if (((gateway[0] != gw_e[0]) || (gateway[1] != gw_e[1]) || (gateway[2] != gw_e[2]) || (gateway[3] != gw_e[3])) &&
    ((gw_e[0] != 0) || (gw_e[1] != 0) || (gw_e[2] != 0) || (gw_e[3] != 0)))
  {
    gateway[0] = gw_e[0];
    gateway[1] = gw_e[1];
    gateway[2] = gw_e[2];
  ...
  if (((subnet[0] != sn_e[0]) || (subnet[1] != sn_e[1]) || (subnet[2] != sn_e[2]) || (subnet[3] != sn_e[3])) &&
    ((sn_e[0] != 0) || (sn_e[1] != 0) || (sn_e[2] != 0) || (sn_e[3] != 0)))
  {
    subnet[0] = sn_e[0];
    subnet[1] = sn_e[1];
    subnet[2] = sn_e[2];
